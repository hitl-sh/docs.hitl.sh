...
- ✅ A HITL.sh account (sign up at [app.hitl.sh](https://app.hitl.sh))
...
url = "https://dev.api.hitl.sh/v1/loops"
headers = {
...
const response = await axios.post('https://dev.api.hitl.sh/v1/loops', {
  name: 'Content Moderation Review',
  description: 'Review user-generated content for community guidelines compliance',
  icon: 'shield-check'
}, {
...
<CodeGroup>
```bash cURL
curl -X POST 'https://dev.api.hitl.sh/v1/loops/YOUR_LOOP_ID/requests' \
  -H 'Authorization: Bearer YOUR_API_KEY' \
  -H 'Content-Type: application/json' \
  -d '{
...
url = f"https://dev.api.hitl.sh/v1/loops/{LOOP_ID}/requests"
headers = {
