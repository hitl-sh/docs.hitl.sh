...
url = "https://dev.api.hitl.sh/v1/api/loops"
headers = {
...
const response = await axios.post('https://dev.api.hitl.sh/v1/api/loops', {
  name: 'Content Moderation Review',
  description: 'Review user-generated content for community guidelines compliance',
  icon: 'shield-check'
}, {
...
```json
{
  ...
  "data": {
    ...
    "qr_code_url": "https://dev.api.hitl.sh/qr/ABC123DEF.png", 
...
<CodeGroup>
```bash cURL
curl -X POST 'https://dev.api.hitl.sh/v1/api/loops/YOUR_LOOP_ID/requests' \
  -H 'Authorization: Bearer YOUR_API_KEY' \
  -H 'Content-Type: application/json' \
  -d '{
...
url = f"https://dev.api.hitl.sh/v1/api/loops/{LOOP_ID}/requests"
headers = {
