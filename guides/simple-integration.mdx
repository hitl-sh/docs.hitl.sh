...
def review_content(content, priority="medium"):
    ...
    response = requests.post(
        f"https://dev.api.hitl.sh/v1/loops/{HITL_LOOP_ID}/requests",
        headers={
...
async function reviewContent(content, priority = 'medium') {
    try {
        ...
        const response = await axios.post(
            `https://dev.api.hitl.sh/v1/loops/${process.env.HITL_LOOP_ID}/requests`,
            {
                processing_type: 'time-sensitive',
                type: 'markdown', 
                priority: priority,
...
RESPONSE=$(curl -s -X POST "https://dev.api.hitl.sh/v1/loops/$HITL_LOOP_ID/requests" \
