...
const response = await fetch('https://dev.api.hitl.sh/v1/loops', {
  headers: {
...
```bash cURL
# Create a test loop
curl -X POST https://dev.api.hitl.sh/v1/loops \
  -H "Authorization: Bearer your_api_key_here" \
...
curl -X POST https://dev.api.hitl.sh/v1/requests \
  -H "Authorization: Bearer your_api_key_here" \
  -H "Content-Type: application/json" \
  -d '{
...
# Create test request
response = requests.post(
    "https://dev.api.hitl.sh/v1/requests",
    headers={
        "Authorization": "Bearer your_api_key_here",
        "Content-Type": "application/json"
    },
...
```javascript JavaScript
const response = await fetch('https://dev.api.hitl.sh/v1/requests', {
  method: 'POST',
  headers: {
    'Authorization': 'Bearer your_api_key_here',
    'Content-Type': 'application/json'
...
def test_rate_limits(api_key):
    ...
    for i in range(105):  # Exceed 100/hour limit
        response = requests.get("https://dev.api.hitl.sh/v1/loops", headers=headers)
