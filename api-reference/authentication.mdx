...
API keys are designed for server-to-server communication and automated workflows. They provide access to the HITL.sh API at `https://dev.api.hitl.sh/v1`.
...
const response = await axios.get('https://dev.api.hitl.sh/v1/api/loops', { headers });
```
...
func main() {
    req, _ := http.NewRequest("GET", "https://dev.api.hitl.sh/v1/api/loops", nil)
...
curl_setopt_array($curl, [
    CURLOPT_URL => "https://dev.api.hitl.sh/v1/api/loops",
...
<Accordion title="401 Unauthorized - Invalid Header Format">
```json
{
  "error": true,
...
<Accordion title="401 Unauthorized - Inactive API Key">
```json
{
  "error": true,
