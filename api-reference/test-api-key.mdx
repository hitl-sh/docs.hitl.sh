---
title: "Test API Key"
api: "GET https://dev.api.hitl.sh/v1/test"
...
response = requests.get("https://dev.api.hitl.sh/v1/test", headers=headers)
data = response.json()
...
async function testApiKey() {
    try {
        const response = await axios.get('https://dev.api.hitl.sh/v1/test', { headers });
...
func testAPIKey(apiKey string) error {
    req, err := http.NewRequest("GET", "https://dev.api.hitl.sh/v1/test", nil)
    if err != nil {
        return err
    }
...
4. **Network issues** - Verify you can reach `dev.api.hitl.sh`
