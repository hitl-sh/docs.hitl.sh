...
api: "GET https://dev.api.hitl.sh/v1/requests"
...
```bash cURL
# Get all requests
curl -X GET https://dev.api.hitl.sh/v1/requests \
  -H "Authorization: Bearer your_api_key_here"

# Get pending requests only
curl -X GET "https://dev.api.hitl.sh/v1/requests?status=pending" \
  -H "Authorization: Bearer your_api_key_here"

# Get high priority requests with pagination
curl -X GET "https://dev.api.hitl.sh/v1/requests?priority=high&limit=20&offset=0" \
  -H "Authorization: Bearer your_api_key_here"
```
...
def get_requests(status=None, priority=None, loop_id=None, limit=50, offset=0):
    ...
    url = "https://dev.api.hitl.sh/v1/requests"
...
def get_requests_dashboard():
    ...
    base_url = "https://dev.api.hitl.sh/v1/requests"
...
async function getRequestAnalytics() {
    ...
    const baseURL = 'https://dev.api.hitl.sh/v1/requests';
...
class RequestsPaginator {
    ...
    async getNextPage(limit = 50) {
        ...
        const response = await axios.get(
            `https://dev.api.hitl.sh/v1/requests?${params}`,
            { headers: { 'Authorization': `Bearer ${this.apiKey}` } }
        );
