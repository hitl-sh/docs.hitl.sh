...
api: "GET https://dev.api.hitl.sh/v1/api/requests"
...
```bash cURL
# Get all requests
curl -X GET https://dev.api.hitl.sh/v1/api/requests \
  -H "Authorization: Bearer your_api_key_here"

# Get pending requests only
curl -X GET "https://dev.api.hitl.sh/v1/api/requests?status=pending" \
  -H "Authorization: Bearer your_api_key_here"

# Get high priority requests with pagination
curl -X GET "https://dev.api.hitl.sh/v1/api/requests?priority=high&limit=20&offset=0" \
  -H "Authorization: Bearer your_api_key_here"
```
...
def get_requests(status=None, priority=None, loop_id=None, limit=50, offset=0):
    ...
    url = "https://dev.api.hitl.sh/v1/api/requests"
...
class RequestsAPI {
    constructor(apiKey) {
        this.apiKey = apiKey;
        this.baseURL = 'https://dev.api.hitl.sh/v1';
    }
...
def get_requests_dashboard():
    ...
    base_url = "https://dev.api.hitl.sh/v1/api/requests"
...
async function getRequestAnalytics() {
    ...
    const baseURL = 'https://dev.api.hitl.sh/v1/api/requests';
...
class RequestsPaginator {
    ...
    async getNextPage(limit = 50) {
        ...
        const response = await axios.get(
            `https://dev.api.hitl.sh/v1/api/requests?${params}`,
            { headers: { 'Authorization': `Bearer ${this.apiKey}` } }
        );
