...
api: "POST https://dev.api.hitl.sh/v1/api/requests/{id}/feedback"
...
```bash cURL
curl -X POST https://dev.api.hitl.sh/v1/api/requests/65f1234567890abcdef12348/feedback \
...
def add_request_feedback(request_id, feedback_data):
    ...
    url = f"https://dev.api.hitl.sh/v1/api/requests/{request_id}/feedback"
    headers = {
...
def analyze_feedback_patterns():
    ...
    response = requests.get(
        "https://dev.api.hitl.sh/v1/api/requests?status=completed&limit=100",
    ...
    for (const { requestId, feedback } of requestFeedbackPairs) {
        try {
            ...
            const feedbackResponse = await axios.post(
                `https://dev.api.hitl.sh/v1/api/requests/${requestId}/feedback`,
                { feedback },
