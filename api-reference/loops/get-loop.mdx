...
api: "GET https://api.hitl.sh/v1/api/loops/{id}"
...
```bash cURL
curl -X GET https://api.hitl.sh/v1/api/loops/65f1234567890abcdef12345 \
...
def get_loop_dashboard():
    ...
    for loop in loops:
        ...
        detail_response = requests.get(f"https://api.hitl.sh/v1/api/loops/{loop['id']}", headers=headers)
...
async function followUpPendingMembers(loopId) {
    const response = await fetch(`https://api.hitl.sh/v1/api/loops/${loopId}`, {
