...
api: "GET https://dev.api.hitl.sh/v1/api/loops/{id}"
...
```bash cURL
curl -X GET https://dev.api.hitl.sh/v1/api/loops/65f1234567890abcdef12345 \
...
def get_loop_dashboard():
    # Get all loops
    loops_response = requests.get("https://dev.api.hitl.sh/v1/api/loops", headers=headers)
    loops = loops_response.json()["data"]["loops"]
    
    dashboard = []
    for loop in loops:
        # Get detailed info for each loop
        detail_response = requests.get(f"https://dev.api.hitl.sh/v1/api/loops/{loop['id']}", headers=headers)
...
async function followUpPendingMembers(loopId) {
    const response = await fetch(`https://dev.api.hitl.sh/v1/api/loops/${loopId}`, {
    ...
    for (const member of pendingMembers) {
        ...
        await sendReminderEmail({
            ...
            loopName: loop.name,
            joinUrl: `https://my.hitl.sh/join/${loop.invite_code}`
        });
    }
