---
title: "Loop Members"
api: "GET https://dev.api.hitl.sh/v1/api/loops/{id}/members"
...
```bash cURL
curl -X GET https://dev.api.hitl.sh/v1/api/loops/65f1234567890abcdef12345/members \
...
url = f"https://dev.api.hitl.sh/v1/api/loops/{loop_id}/members"
headers = {
...
async function getLoopMembers(loopId) {
    const response = await axios.get(
        `https://dev.api.hitl.sh/v1/api/loops/${loopId}/members`,
        {
            headers: {
                'Authorization': 'Bearer your_api_key_here'
            }
...
api: "DELETE https://dev.api.hitl.sh/v1/api/loops/{id}/members/{userId}"
...
**Example Request:**
```bash
curl -X DELETE https://dev.api.hitl.sh/v1/api/loops/65f1234567890abcdef12345/members/65f1234567890abcdef12347 \
  -H "Authorization: Bearer your_api_key_here"
```
...
def follow_up_pending_members(loop_id, days_threshold=3):
    ...
    response = requests.get(f"https://dev.api.hitl.sh/v1/api/loops/{loop_id}/members", headers=headers)
...
def audit_loop_membership(loop_ids):
    ...
    for loop_id in loop_ids:
        try:
            # Get loop info
            loop_response = requests.get(f"https://dev.api.hitl.sh/v1/api/loops/{loop_id}", headers=headers)
            loop_data = loop_response.json()["data"]["loop"]
            
            # Get members
            members_response = requests.get(f"https://dev.api.hitl.sh/v1/api/loops/{loop_id}/members", headers=headers)
