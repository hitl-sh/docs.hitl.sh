---
title: "Delete Loop"
api: "DELETE https://dev.api.hitl.sh/v1/api/loops/{id}"
...
```bash cURL
curl -X DELETE https://dev.api.hitl.sh/v1/api/loops/65f1234567890abcdef12345 \
...
url = f"https://dev.api.hitl.sh/v1/api/loops/{loop_id}"
headers = {
...
if (confirmed) {
    const response = await axios.delete(
        `https://dev.api.hitl.sh/v1/api/loops/${loopId}`,
        {
            headers: {
                'Authorization': 'Bearer your_api_key_here'
            }
...
def safe_delete_loop(loop_id, require_confirmation=True):
    ...
    response = requests.get(f"https://dev.api.hitl.sh/v1/api/loops/{loop_id}", headers=headers)
    ...
    requests_response = requests.get(f"https://dev.api.hitl.sh/v1/api/loops/{loop_id}/requests", headers=headers)
    ...
    delete_response = requests.delete(f"https://dev.api.hitl.sh/v1/api/loops/{loop_id}", headers=headers)
...
async function safeDeleteLoop(loopId, requireConfirmation = true) {
    try {
        // Get loop information first
        const loopResponse = await axios.get(`https://dev.api.hitl.sh/v1/api/loops/${loopId}`, {
        ...
        try {
            const requestsResponse = await axios.get(`https://dev.api.hitl.sh/v1/api/loops/${loopId}/requests`, {
                headers: { 'Authorization': `Bearer ${apiKey}` }
            });
        ...
        // Proceed with deletion
        const deleteResponse = await axios.delete(`https://dev.api.hitl.sh/v1/api/loops/${loopId}`, {
            headers: { 'Authorization': `Bearer ${apiKey}` }
        });
    ...
    for loop_id in loop_ids:
        try:
            if dry_run:
                ...
                response = requests.delete(f"https://dev.api.hitl.sh/v1/api/loops/{loop_id}", headers=headers)
                if response.status_code == 200:
                    results.append({
                        "loop_id": loop_id,
                        "status": "deleted",
...
def archive_loop(loop_id):
    ...
    update_response = requests.put(
        f"https://dev.api.hitl.sh/v1/api/loops/{loop_id}",
        headers=headers,
        json={
            "name": archived_name,
...
async function archiveLoop(loopId) {
    // Get current loop data
    const response = await axios.get(`https://dev.api.hitl.sh/v1/api/loops/${loopId}`, {
        headers: { 'Authorization': `Bearer ${apiKey}` }
    });
