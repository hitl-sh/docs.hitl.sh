---
title: "Update Loop"
api: "PUT https://dev.api.hitl.sh/v1/api/loops/{id}"
...
```bash cURL
curl -X PUT https://dev.api.hitl.sh/v1/api/loops/65f1234567890abcdef12345 \
...
def standardize_loop_names(loop_prefix="Review"):
    ...
    for loop in loops:
        ...
        update_response = requests.put(
            f"https://dev.api.hitl.sh/v1/api/loops/{loop['id']}", 
            headers=headers, 
            json={"name": new_name}
        )
...
def version_loop_update(loop_id, updates, version=None):
    ...
    return requests.put(f"https://dev.api.hitl.sh/v1/api/loops/{loop_id}", headers=headers, json=updates)
```
...
def safe_loop_update(loop_id, updates):
    # Get current state
    current_response = requests.get(f"https://dev.api.hitl.sh/v1/api/loops/{loop_id}", headers=headers)
