...
HITL.sh supports five content types for human review:
...
Review visual content like photos, graphics, or screenshots requiring human evaluation. Upload supported (10MB max)
</Card>

<Card title="Document" icon="file">
Review files such as PDFs, DOCX, and other documents via `file_url`, `file_type`, and `file_name`. Upload supported (50MB max)
</Card>

<Card title="Video Link" icon="video">
Review video content via `video_url`. Supports YouTube, Vimeo, or raw .mp4/.webm/.mov links. Link only — no upload
</Card>

<Card title="Audio Link" icon="music">
Review audio content via `audio_url`. Supports SoundCloud, Spotify, or raw .mp3/.wav/.ogg/.m4a/.aac links. Link only — no upload
</Card>
</CardGroup>
...
Use `type: "file"` for document review:

```python
request_data = {
    "type": "file",
    "request_text": "Review this contract for compliance issues",
...
Use `type: "video"` for video content review:

```python
request_data = {
    "type": "video",
    "request_text": "Review this training video for accuracy and completeness",
...
### Audio Link Content Type

Use `type: "audio"` for audio content review:

```python
request_data = {
    "type": "audio",
    "request_text": "Review this customer service call for quality assurance",
...
```json
{
  ...
  "type": "markdown|image|file|video|audio",
...
Review a file such as a contract or compliance document:
...
Review a video for accuracy and quality:

```python
import requests

# Create a video review request
request_data = {
    "processing_type": "deferred",
    "type": "video",
    "priority": "low",
...
Review an audio recording for quality assurance:

```python
import requests

# Create an audio review request
request_data = {
    "processing_type": "time-sensitive",
    "type": "audio",
    "priority": "high",
