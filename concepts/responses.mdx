...
Responses are the structured decisions and feedback that human reviewers provide when completing HITL.sh requests. The response system supports five different types, each designed for specific use cases and providing different levels of structure and validation.
...
HITL.sh supports five distinct response types, each optimized for different decision-making scenarios:
...
"response_config": {
    ...
    # Simple strings automatically converted to rich SelectOption objects
}
```
...
Detailed documentation of all five response types with examples
</Card>
