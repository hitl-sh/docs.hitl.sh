...
- **Name**: Clear, descriptive identifier for the loop (required, max 100 characters)
- **Description**: Detailed explanation of the loop's purpose and scope (optional, max 500 characters)
- **Icon**: Visual identifier for the loop (required, max 100 characters - typically an emoji or short text)
...
loop_response = response.json()["data"]
loop = loop_response["loop"]
print(f"Loop created: {loop['name']}")
print(f"Invite code: {loop_response['invite_code']}")
print(f"QR code URL: {loop_response['qr_code_url']}")
print(f"Join URL: {loop_response['join_url']}")
```
...
async function createLoop() {
    ...
    try {
        ...
        console.error("❌ Failed to create loop:", error.response?.data);
    }
}
```
...
response = requests.delete(
    f"https://dev.api.hitl.sh/v1/api/loops/{loop_id}/members/{member_id}",
    headers={"Authorization": f"Bearer {api_key}"}
)
...
if response.status_code == 200:
    ...
    print("❌ Removal failed:", response.json())
```
...
members_response = requests.get(
    f"https://dev.api.hitl.sh/v1/api/loops/{loop_id}/members",
    headers={"Authorization": f"Bearer {api_key}"}
)
...
for member in inactive_members:
    ...
    requests.delete(
        f"https://dev.api.hitl.sh/v1/api/loops/{loop_id}/members/{member['user_id']}",
        headers={"Authorization": f"Bearer {api_key}"}
    )
```
...
**Solutions**:
- **Only loop creators can manage membership** - including removing members, updating loop details, and deleting loops
- Loop creators cannot remove themselves from the loop
